...
Mesh::Mesh(Vertex* vertices, int vertexCount, unsigned int* indices, int indexCount, Microsoft::WRL::ComPtr<ID3D12Device> device)
{
	this->indexCount = indexCount;

	CalculateTangents(vertices, vertexCount, indices, indexCount);

	MeshHelper(vertices, vertexCount, indices, indexCount, device);
}

Mesh::Mesh(const char* filePath, Microsoft::WRL::ComPtr<ID3D12Device> device)
{
	...
	MeshHelper(&verts[0], vertCounter, &indices[0], indexCounter, device);
}

Mesh::~Mesh()
{
	// nothing needs to be done because we're using com pointers
}

void Mesh::MeshHelper(Vertex* vertices, int vertexCount, unsigned int* indices, int indexCount, Microsoft::WRL::ComPtr<ID3D12Device> device)
{
